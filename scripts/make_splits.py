...
def main():
    ...
    ap.add_argument(
        "--data_root", type=str, required=True, help="e.g., data/chexpert_small"
    )
    ...
    df_train = df_train_full[df_train_full["patient_id"].isin(train_p)].reset_index(
        drop=True
    )
    df_dev = df_train_full[df_train_full["patient_id"].isin(dev_p)].reset_index(
        drop=True
    )
    ...
    print(
        f"Saved splits to {out_dir} | train={len(df_train)} dev={len(df_dev)} test={len(df_test)}"
    )
